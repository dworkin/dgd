...
void MainFrame::MessageReceived(BMessage *message)
{
    ...
    case DGD_START:
	dgd_started = true;
	resume_thread(spawn_thread(run_dgd, "driver", B_NORMAL_PRIORITY, NULL));
	break;
...
{
    config->SetEnabled(!dgd_started);
    restore->SetEnabled(!dgd_started);
    restore->SetMarked(restore_file.Path() != NULL);
    start->SetEnabled(!dgd_started && config_file.Path() != NULL &&
...
void DGD::AboutRequested(void)
{
    ...
    about->AddChild(view);
    sprintf(buf, "DGD %s\n© 1993 - 1998 BeeHive Internet Technologies, Inc. All Rights Reserved.", VERSION);
    view->SetText(buf);
