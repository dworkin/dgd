/*
 * This file is part of DGD, http://dgd-osr.sourceforge.net/
 * Copyright (C) 1993-2010 Dworkin B.V.
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */

/*
 * Definitions for the TELNET protocol.
 */

# define IAC		(unsigned char) 255	/* interpret as command */
# define DONT		(unsigned char) 254	/* don't */
# define DO		(unsigned char) 253	/* do */
# define WONT		(unsigned char) 252	/* won't */
# define WILL		(unsigned char) 251	/* will */
# define SB		(unsigned char) 250	/* begin subnegotiation */
# define GA		(unsigned char) 249	/* go ahead */
# define AYT		(unsigned char) 246	/* are you there */
# define IP		(unsigned char) 244	/* interrupt process */
# define BREAK		(unsigned char) 243	/* break */
# define SE		(unsigned char) 240	/* end subnegotiation */

/* options */
# define TELOPT_ECHO	(unsigned char) 1	/* echo */
# define TELOPT_SGA	(unsigned char) 3	/* suppress go ahead */
# define TELOPT_TM	(unsigned char) 6	/* timing mark */
