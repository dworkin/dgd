...
void conn_listen()
{
    ...
    for (n = 0; n < nudescs; n++) {
	nonblock = TRUE;
	if (udescs[n].fd.in6 != INVALID_SOCKET &&
	    ioctlsocket(udescs[n].fd.in6, FIONBIO, &nonblock) != 0) {
	    fatal("ioctlsocket failed");
	}
	nonblock = TRUE;
	if (udescs[n].fd.in4 != INVALID_SOCKET &&
	    ioctlsocket(udescs[n].fd.in4, FIONBIO, &nonblock) != 0) {
	    fatal("ioctlsocket failed");
	}
    }
}
